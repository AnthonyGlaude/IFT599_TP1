...
def compute_hits(G: nx.DiGraph, max_iter: int = 100, tol: float = 1.0e-8):
    try:
        hub_scores, authority_scores = nx.hits(G, max_iter=max_iter, normalized=True)
...
def index():
    ...
    if request.method == "POST":
        ...
        if critere == "PageRank":
            ...
            start_time = time.time()
            hub_scores, authority_scores = compute_hits(G)
            execution_time = time.time() - start_time
            top_auth = sorted(authority_scores.items(), key=lambda x: x[1], reverse=True)[:K]
            scores["HITS-autorite"] = {
                "data": top_auth,
                "time": execution_time,
                "graph_size": graph_size
            }
